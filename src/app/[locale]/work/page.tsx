          ...
          {t.work.items.map((c) => (
            <Link key={c.slug} href={localePath(locale, `/work/${c.slug}`)} className="group">
              <Card className="flex h-full flex-col p-4 sm:p-5">
                <PillarArt slug={c.pillar} title={c.title} crop className="aspect-[5/2]" />
                ...
                <h2 className="mt-4 px-2 text-xl font-semibold leading-snug">{c.title}</h2>
                <p className="mt-3 flex-1 px-2 text-muted">{c.summary}</p>
                <span className="mt-6 inline-flex items-center gap-1.5 px-2 pb-2 text-sm font-medium text-accent">
                  {t.common.learnMore}
